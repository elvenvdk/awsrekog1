...
exports.handler = (event, context, callback) => {
  //Configuration - Start
  const srcBucket = "detect-face";
  const managerPhoneNumber = "+19173066656";
  const table = "demographic";
  //Configuration - End

  const srcKey = decodeURIComponent(event.filename.replace(/\+/g, " "));
  const params = {
    Image: {
      S3Object: {
        Bucket: srcBucket,
  ...
  rekognition.detectFaces(params, function(err, result) {
    if (err) {
      ...
      if (result !== null) {
        ...
        const id = uuidV4(); //generate a UUID number
        const paramsDB = {
          TableName: table,
          Item: {
            id: id,
        ...
        docClient.put(paramsDB, function(err, rs) {
          if (err) {
            ...
            if (result.FaceDetails !== null && result.FaceDetails.length > 0) {
              const item = result.FaceDetails[0];
              if (item !== null && item.Confidence >= 80) {
                if (item.Emotions !== null && item.Emotions.length > 0) {
                  const pe = item.Emotions[0]; //most pre-dominant emotion - emotions array is reverse sorted by confidence
                  if (
                    pe.Confidence > 80 &&
                    (pe.Type.toLowerCase() == "angry" ||
                      pe.Type.toLowerCase() == "sad" ||
                      pe.Type.toLowerCase() == "confused" ||
                      pe.Type.toLowerCase() == "disgusted") &&
                    item.AgeRange.Low > 15
                  ) {
                    //contact store manager by sending a text message
                    //generate tinyurl
                    const url =
                      "http://tinyurl.com/api-create.php?url=https://s3.amazonaws.com/" +
                      srcBucket +
                      "/" +
                      srcKey;
                    http
                      .get(url, function(res) {
                        ...
                        res.on("end", function() {
                          const shortenedurl = body;
                          const paramsmessage = {
                            Message:
                              "customer: " +
                              pe.Type.toLowerCase() +
                              ", gender: " +
                              item.Gender.Value.toLowerCase() +
                              ", age range: " +
                              item.AgeRange.Low +
                              "-" +
                              item.AgeRange.High +
                    ...
                    if (item.AgeRange.High <= 15) {
                      ...
                      if (item.Gender.Confidence > 80) {
                        if (item.Gender.Value == "Female") {
                          ...
                          //male
                          if (
                            (item.Mustache.Confidence > 80 &&
                              item.Mustache.Value === true) ||
                            (item.Beard.Confidence > 80 &&
                              item.Beard.Value === true)
                          ) {
